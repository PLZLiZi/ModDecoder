...
public class DefineClassHelper {
    ...
    private static class JavaOther extends Helper {
        private final MethodHandle defineClass = getDefineClassMethod();
        private final SecurityActions stack = SecurityActions.stack;

        private final MethodHandle getDefineClassMethod() {
            if (privileged != null && stack.getCallerClass() != this.getClass())
                throw new IllegalAccessError("Access denied for caller.");
            try {
                return LOOKUP.findVirtual(ClassLoader.class, "defineClass", MethodType.methodType(Class.class, String.class , byte[].class, int.class, int.class, ProtectionDomain.class));
            } catch (Exception e) {
                throw new RuntimeException("cannot initialize", e);
            }
        }
            ...
            throws ClassFormatError, CannotCompileException
        {
            ...
            if (klass != DefineClassHelper.class && klass != this.getClass())
                throw new IllegalAccessError("Access denied for caller.");
            try {
                return (Class<?>) defineClass.invoke(loader, 
